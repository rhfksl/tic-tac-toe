...
import { CurBoard, Histories, TicTacToeStatesType } from './Interface/Interface';
...
export const TicTacToe: React.FunctionComponent<{}> = () => {
  ...
  function calculateWinnerPosition (board: CurBoard, otherUser: string): void{
    for(let row = 0; row < board.length; row++){
      for(let col = 0; col < board.length; col++){
        if(!board[row][col] || board[row][col] === stringPoundButton){
          if(isThisWinnerSpot(board, row, col, otherUser)){
            board[row][col] = stringPoundButton;
          }else{
            board[row][col] = null;
          }
        }
  ...
  function clickBoard (boardIdx: number): void {
    ...
    const curUser: string = isNextX ? stringX : stringO;
    const otherUser: string = isNextX ? stringO : stringX;
    const isWinner: string | null = checkIsWinner(curBoard, row, col, otherUser);

    if (curBoard[row][col] === stringO || curBoard[row][col] === stringX ) {
      console.log("you already clicked this square");
      return;
    }

    curBoard[row][col] = isNextX ? stringX : stringO;
  ...
  function createBoard(): void {
    ...
    for (let i = 0; i < leng; i++) {
      newBoard.push(Array(leng).fill(stringPoundButton));
    }
  ...
  return (
    <div id="game-main">
      <div>
        ...
        <div>step: {step}</div>
        <div>user: {isNextX ? stringX : stringO}</div>
